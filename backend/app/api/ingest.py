...
def ingest_meal_log(payload: MealLogIngestRequest, db: Session = Depends(get_db)) -> IngestResult:
    ...
    for row in payload.rows:
        ...
        # PRD 6.1/6.3: mealdata.csv에는 메뉴명이 없으므로(코너만 있음), 같은 날·같은
        # 식사구분·같은 코너의 "메인" 메뉴를 그 날 실제 제공 메뉴로 간주해 연결한다.
        ...
        db.add(
            MealLog(
                ...
                meal_type=row.meal_type,
                corner_id=corner.corner_id,
                menu_id=menu_snapshot.menu_id if menu_snapshot else None,
                taste_score=row.taste_score,
                comment=row.comment,
                menu_snapshot_id=menu_snapshot.id if menu_snapshot else None,
            )
        )
        inserted += 1

    db.commit()
    return IngestResult(received=len(payload.rows), inserted=inserted)
