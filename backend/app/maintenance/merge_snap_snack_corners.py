"""스냅스낵/스냅스넥으로 이미 나뉘어 적재된 기존 데이터를 하나의 코너로 합친다.

`app/services/master_data.py`의 코너명 정규화는 **앞으로 들어오는** 취식기록에만
적용된다. 정규화가 배포되기 전에 이미 적재된 과거 데이터는 corner_master에
"스냅스낵"/"스냅스넥"이 서로 다른 corner_id로 남아있으므로, 이 스크립트로 한 번
병합해야 한다 (merge_take_out_corners.py와 같은 구조, 2026-08).

사용법 (backend/ 디렉토리에서):
    python -m app.maintenance.merge_snap_snack_corners
...
from app.services.master_data import SNAP_SNACK_ALIASES, SNAP_SNACK_CORNER_NAME, get_or_create_corner


def merge_snap_snack_corners(db: Session) -> int:
    """returns: 재배정된 meal_log 행 수."""
    alias_corners = db.query(CornerMaster).filter(CornerMaster.corner_name.in_(SNAP_SNACK_ALIASES)).all()
    if not alias_corners:
        return 0

    canonical, _ = get_or_create_corner(db, SNAP_SNACK_CORNER_NAME)
    reassigned = 0
...
def run() -> None:
    db = SessionLocal()
    try:
        reassigned = merge_snap_snack_corners(db)
        if reassigned == 0:
            print("병합할 스냅스낵/스냅스넥 별칭 코너가 없습니다 (이미 병합됐거나 해당 없음).")
            return
        print(f"✅ 스냅스낵/스냅스넥을 '{SNAP_SNACK_CORNER_NAME}' 코너로 병합했습니다 (meal_log {reassigned}행 재배정).")
