...
```
두 가지를 출력한다:
1. **자동 진단(복붙 불필요)** — 조인 키 4개 필드(ID/날짜/식사구분/메뉴명) 중
   하나씩 빼고 다시 세어봐서, 어느 필드가 매칭을 막고 있는지 **숫자로** 보여준다.
   예: "ID만 무시하고 매칭: 480행"인데 "전부 일치: 0행"이면 ID(사번/Knox ID)가
   원인. 화면에 뜬 숫자만 읽어서 알려줘도 원인을 좁힐 수 있다(사내망이라 복붙이
   안 되는 환경 고려).
2. **원본 샘플** — 취식기록/맛평가 양쪽에서 각각 5건씩(숫자 지정 가능,
