...
자동변환하는 문제 등), `--debug-sample`로 실제 비교되는 조인 키를 그대로 출력해서
확인할 수 있다:
```powershell
python cli.py meal-log "transactions.xlsx" "taste_eval.xlsx" --debug-sample
```
취식기록/맛평가 양쪽에서 각각 5건씩(숫자 지정 가능, `--debug-sample 10`) 조인 키
`(사번 또는 Knox ID, 날짜, 식사구분, 메뉴명)`을 `repr()`로 출력한다 — 육안으로는
같아 보이지만 실제로는 다른 값(공백, 타입 차이 등)을 그대로 드러낸다.
