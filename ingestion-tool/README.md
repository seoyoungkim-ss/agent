...
```powershell
# 주간 식단표 1개 — 어느 주인지는 시트 헤더의 날짜에서 자동으로 알아낸다
...
모든 명령이 전송 전 파싱/병합 결과(행 수, 맛평가 매칭률, 샘플, 경고)를
보여주고 `y`를 입력해야 백엔드로 전송한다. `--yes`를 주면 확인 없이 바로 전송한다.

### 주간 식단표 여러 개를 한 번에 (`weekly-menu-batch`)

폴더에 있는 식단표를 통째로 올린다. 8개월치 소급 적재처럼 파일이 수십 개일 때 쓴다.

```powershell
# 1) 먼저 --dry-run 으로 "어느 파일이 어느 주로 읽혔는지"만 확인한다
python cli.py weekly-menu-batch "C:\식단표" --dry-run

# 2) 표가 맞으면 --dry-run 없이 다시 실행해 전송
python cli.py weekly-menu-batch "C:\식단표"
```

- **주차는 시트 헤더의 날짜에서 자동 인식한다.** 파일명 규칙에 의존하지 않는다.
  확신이 없으면 값을 만들어내지 않고 그 파일만 건너뛴다 — 나머지는 계속 올라간다.
- **여러 번 돌려도 중복이 쌓이지 않는다.** 같은 주가 이미 있으면 교체하기 때문에,
  일부 파일만 실패했을 때 폴더째 다시 돌려도 된다.
  단, **화면에서 직접 고친 주찬/부찬·건강가든 행은 교체 대상에서 제외**되므로
  수동 수정이 재업로드로 날아가지 않는다.
- 엑셀로 열어둔 파일이 만드는 `~$...` 잠금 파일은 자동으로 무시한다.
- Excel 인스턴스를 하나만 띄워 재사용한다(파일마다 띄우면 31개에 31번 뜬다).

> ⚠️ **`--dry-run` 표를 먼저 눈으로 확인하라.** 주를 잘못 읽으면 그 주 편성이
> 통째로 어긋날 뿐 아니라, 교체 방식이라 **멀쩡한 다른 주를 덮어쓴다.**
> 표에서 한 줄이라도 이상하면 진행하지 말고 그 파일만 따로
> `weekly-menu ... --week-start`로 올린다.

자동 인식이 안 되는 파일(헤더에 날짜가 없는 옛 양식 등)은 단건 명령에서
직접 지정한다:

```powershell
python cli.py weekly-menu "C:\식단표\옛양식.xlsx" --week-start 2026-07-20 --replace-existing
```
...
2. **자동 진단 B — 취식기록 기준(참고용)** — 같은 방식이지만 취식기록 건수를
   ...
   우선 봐야 한다.
3. **자동 진단 C — 필드별 값 비교 샘플** — 진단 A에서 특정 필드(예: ID)가
   원인으로 좁혀졌을 때, 아직 안 맞은 맛평가마다 "그 필드만 빼면 매칭되는
   취식기록"을 찾아 그 필드의 **실제 값을 맛평가 쪽과 취식기록 쪽 나란히**
   보여준다(필드당 최대 5건, `--debug-sample`로 개수 조절). 예:
   `맛평가='12345678' vs 취식기록='012345678'`처럼 자릿수/공백 차이를 그
   자리에서 읽어서 알려줄 수 있다.
4. **원본 샘플** — 취식기록/맛평가 양쪽에서 각각 5건씩(숫자 지정 가능,
   `--debug-sample 10`) 조인 키를 `repr()`로 그대로 출력한다 — 위 진단들로
   원인 필드를 좁힌 뒤에도 더 봐야 할 게 있으면 이걸로 확인한다.

> ⚠️ 수동 실행형 도구다. 새 식단표/취식 데이터를 받을 때마다 운영자가 직접
> 실행해야 하며, 실행을 놓치면 데이터 공백이 생긴다.
>
> 다만 **주간 식단표는 공백이 생겨도 복구가 쉽다** — 밀린 파일을 한 폴더에 모아
> `weekly-menu-batch`로 한 번에 올리면 되고, 이미 올린 주가 섞여 있어도 교체되므로
> 중복이 쌓이지 않는다. 취식 데이터(`meal-log`)는 아직 파일 단위 실행이라
> 여전히 체크리스트로 관리하는 것을 권장한다.
