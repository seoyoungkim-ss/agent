...
## §121 — VOE 분류 최다 코너·메뉴 하이라이트/클릭 펼치기 + VOE 클러스터링 네트워크 도식화

### Context

담당자 요청: "voe 분류시 최다 코너/최다메뉴 하이라이트 해주고 클릭하면
관련내용 펼쳐지게 해줘" + "voe 클로스터링도 이미지처럼 도식화시켜서
알아보기 쉽게 하고싶어 현재는 아무런 쓸모가업ㄱ음"(참고 이미지: 중심
원 하나에 주제별 원이 방사형으로 연결된 "토픽 클러스터 네트워크" 다이어그램).

`VoeAnalysisTab`(`AnalysisPage.tsx`)의 "VOE 분류" 섹션은 이미
`computeTopVoeEntries`로 최다 코너/메뉴를 계산해 `StatTile`로 보여주고
있었지만 클릭이 안 됐고(정적 표시만), "VOE 클러스터링" 섹션은 그냥
카드 그리드 나열이라 한눈에 패턴이 안 보였다(담당자가 "쓸모없다"고
느낀 이유).

### 설계

**1. VOE 분류 최다 코너·메뉴 하이라이트 + 클릭 펼치기.**
`computeTopVoeEntries`가 반환하던 `{topVoeCorner, topVoeMenu}`에
중복제거된 코멘트 배열(`comments`)도 함께 반환하도록 확장 — 새 쿼리
없이 이미 계산해둔 값을 재사용한다. 두 `StatTile`에 `tone="warning"`
(하이라이트 표시, 노란 점+왼쪽 테두리)과 `onClick`을 추가해 클릭하면
`selectedVoeHighlight: {month, kind: "corner"|"menu", value}` 상태를
토글하고, 그 아래 코너/메뉴로 필터링한 코멘트 `Table`을 펼친다(기존
카테고리 클릭·펼치기와 같은 패턴). 하이라이트 선택과 카테고리 선택은
서로 배타적으로 만들어(한쪽을 클릭하면 다른 쪽은 닫힘) 한 번에 하나의
코멘트 목록만 보이게 했다.

**2. VOE 클러스터링 — 허브·스포크 네트워크 다이어그램.**
카드 그리드 대신 ECharts `graph` 시리즈(`layout: "force"`)로 그린다 —
§19에서 코너 코어층 메뉴쌍에 쓰다 §82에서 화면과 함께 삭제된
`buildMenuPairGraphOption`과 같은 패턴(현재는 코드에 없어 git 이력에서
복원해 참고)을 새 함수 `buildVoeClusterGraphOption`으로 재구현했다.
중심 노드(그 달 라벨, 예: "2026년 8월") 하나에 클러스터별 노드가
연결되고, 노드 크기는 `comment_count`에 비례(`26 + (count/max)*34`),
간선 두께도 비례해서 굵기로 비중이 보인다. 마우스오버 시 툴팁으로
라벨·키워드·건수를 보여주고, 노드를 클릭하면(중심 노드 클릭은 무시)
`selectedVoeCluster: {month, label}` 상태로 그래프 아래에 상세 패널
(키워드/건수/대표 코멘트)을 띄운다 — 기존 카드가 담던 정보를 그대로
유지하면서 "패턴이 한눈에 보이는" 형태로 바꾼 것.

구현 중 버그 하나를 잡았다: 중심 노드를 `fixed: true, x: 0, y: 0`으로
고정했더니 그래프가 캔버스 왼쪽 위 모서리에 뭉쳐버렸다 — `force`
레이아웃은 별도 좌표계가 없어 `x`/`y`가 그대로 픽셀 좌표로 쓰이기
때문(원점 0,0은 캔버스 좌상단이지 중앙이 아님). 위치 고정을 빼고 물리
시뮬레이션에 맡기니, 모든 클러스터와 연결된 중심 노드가 인력·척력
균형으로 자연스럽게 가운데 쪽에 자리잡았다(`force.repulsion=220`,
`gravity=0.3`으로 살짝 조정).

`useChartTheme()`(`ui.tsx`)에 `surface`(카드 배경색, 라이트/다크 모두
대응) 필드를 추가해 중심 노드 배경으로 썼다 — 스포크 노드(accent
색)와 시각적으로 구분되면서도 테마에 맞게 자동 대응된다.

### 손대지 않는 것 (교차 확인)

- `GET /dashboard/voe-by-category`/`GET /dashboard/voe-clusters`
  백엔드 엔드포인트·응답 스키마 — 무변경. 최다 코너/메뉴 계산은 이미
  프론트에서 하고 있던 로직을 재사용했을 뿐, 백엔드에 코너/메뉴별
  집계 엔드포인트를 새로 만들지 않았다(§120의 VOE 브리핑 facts 확장과는
  별개 — 그건 브리핑 요약 문장용, 이건 화면 표시용).
  - `cluster_monthly_voe`(월간 클러스터링 계산/저장 로직) — 무변경,
    시각화만 바뀜.
  - "주관식 VOE" 4분면 차트, "월별 VOE 코멘트 수 추이" 라인차트 — 무변경.

### 테스트/검증

- 프론트 전용 변경(백엔드 무변경) — `pytest` 전체 578개 통과(§120과
  동일하게 무관 사전 실패 2개 제외, 회귀 없음 확인).
- `npx tsc -b` + `npx vite build` 클린.
- **실제 개발 DB + Playwright**: (1) "이 달 VOE 최다 코너"(한식, 29건)
  타일을 클릭해 코너 "한식" 관련 코멘트 29건이 담긴 표가 펼쳐짐을
  확인, (2) 개발 DB에 LLM 모의 응답을 여러 그룹으로 만들어(사내 LLM
  미설정 환경이라 기본 모의 응답은 항상 클러스터 1개뿐이라 검증용으로
  임시 monkeypatch) 클러스터 4개를 시딩 → 네트워크 그래프가 중심
  노드("2026년 8월") + 4개 스포크 노드로 정상 렌더링되고 중앙에 잘
  배치됨을 확인(수정 전엔 좌상단에 뭉쳐 있었음), (3) 스포크 노드
  클릭 시 툴팁과 함께 아래 상세 패널(키워드/건수/대표 코멘트)이 뜸을
  확인, 중심 노드 클릭은 무시됨을 확인. 콘솔 에러 0건.
- 문서화(§121) 후 커밋·푸시.

## §122 — 주관식 VOE 탭 전면 개편(단일 기준월 대시보드)

### Context

담당자 요청: "주관식 VOE 탭을 한 페이지에서 '뭐가 문제인지' 바로
파악할 수 있는 구조로 개편해줘" — 요약 카드 3개(다크 강조, 숫자
32~40px) → 인사이트 문장+최근 2주 추이 → 주요 이슈 클러스터 태그 →
코너별 막대+VOE 리스트(코너·태그 AND 필터), 이 순서로 위에서 아래로
"근거"가 쌓이는 구조를 정확한 스펙으로 지정. §121까지는 기준월+표시
개월수(1/3/6/12개월)로 여러 달을 리스트로 늘어놓는 화면이었는데, 이번
요청은 "숫자 3개만 봐도 이번 달 상황 요약"이 목표라 다중월 리스트와는
근본적으로 다른 구조 — 단일 기준월 중심으로 전면 재구성했다.

### 설계

**데이터는 기존 API·순수 함수를 그대로 재사용, 신규 백엔드 없음.**
`voeByCategory`(기준월+전월 2번 호출 — MoM 비교/최근 2주 창용),
`voeClusters`, `voeBriefing`, `cornerList`, 그리고 `computeTopVoeEntries`
(§121에서 이미 comments도 반환하도록 확장돼 있어 그대로 사용)만 쓴다.

**블록1(요약 카드 3개+AI 브리핑).** `StatTile`에 opt-in `size="lg"`
prop을 추가해 값 텍스트를 28px→36px로 키웠다(기존 호출부는 `size`
안 넘기면 그대로 28px, 무변경). 3개 다 `variant="dark"`(기존
`--hero-bg` #0f1b33 — 요청한 #0F172A와 사실상 동일한 다크 네이비)로
"다크 배경 흰 카드 사이 시각적 앵커" 요청을 만족한다. 세 카드 모두
숫자가 가장 큰 값이 되도록 통일했다 — 총 건수는 건수 자체, 최다
코너/메뉴는 "이름"이 아니라 "건수"를 큰 숫자로 놓고 이름은 `sub`로
작게(요청의 "숫자는 32~40px으로 가장 크게" 지시를 세 카드에 일관 적용).
총 건수 카드의 `trend`는 HomePage의 `todayHeadcountTrend`와 같은 계산
관례(±1%는 "―")를 따르되, VOE는 늘어나는 게 나쁜 신호라 up=warning/
down=good으로 톤을 반전시켰다. 최다 코너/메뉴 카드는 클릭하면
`selectedCorner`/`selectedMenu`를 토글해 블록4 리스트와 바로 연동된다.
AI 브리핑은 3개 카드 바로 아래, 같은 카드 안에 배치(요청 "3개
아래에는 ai브리핑을 적어줘"를 문자 그대로 반영) — 기존 재계산 로직
그대로.

**블록2(인사이트 문장+최근 2주 추이).** 기준월이 이번 달이면 "오늘",
지난달을 골랐으면 "그 달 마지막 날"을 `asOf`로 잡고 최근 14일/그 이전
14일을 비교한다. 새 API 없이 이미 받아온 기준월+전월 코멘트를 합쳐
필터링(월 경계를 걸치는 14일 창 대비). 인사이트 문장은 (a) 전 2주
대비 증감률 20%를 급증/급감 문턱으로, (b) 최근 2주 중 코멘트가 가장
많은 코너, (c) 특정 클러스터의 라벨/키워드가 최근 2주 코멘트의 40%
이상(최소 2건)을 차지하면 "그 주제가 급증을 주도했다"고 보고 문장에
곁들이되, 근거가 약하면(문턱 미달) 조용히 생략한다 — §120 VOE
브리핑에서 세운 "사실에 없는 내용은 지어내지 않는다" 원칙을 이
자동생성 문장에도 그대로 적용했다. 그래프는 일별 코멘트 수 라인차트
+ 평균의 1.6배 이상(최소 2건)인 날에 `markPoint`(핀 모양) 콜아웃을
최대 2개까지 표시("급증/급감 구간" 표시 요청).

**블록3(주요 이슈 클러스터 태그).** `voeClusters` 결과를 건수 내림차순
정렬해 상위 6개는 태그로, 나머지는 "+N개 더 보기"로 접는다(이 코드베이스
전역에서 이미 쓰는 `showAllX`/`visibleX` 프리뷰 관례 재사용). 태그
클릭 시 `selectedClusterTag`를 토글.

**주의:** `MonthlyVoeCluster`는 라벨·키워드·건수·대표 코멘트 1건만
저장하고 "어느 코멘트가 그 클러스터에 속하는지"는 저장하지 않는다
(`voe_clustering.py` — LLM이 그룹만 만들고 배정을 DB에 남기지 않음).
그래서 "태그 클릭 → 리스트 필터링"(블록3↔4 연동)은 정확한 소속 매칭이
아니라 **근사치**다 — `commentMatchesCluster()`가 그 클러스터의
라벨/키워드가 코멘트 본문에 실제 등장하는지로 판정한다. 실사용 확인
중 키워드가 1글자(예: "간")일 때 무관한 단어(예: "대기시간")에도
부분 문자열로 걸리는 오탐을 발견해, 키워드 최소 길이 2글자 조건을
추가했다(2026-09).

**블록4(좌: 코너별 막대 / 우: VOE 리스트, AND 필터).** 코너 막대는
`CornerAnalysisTab` 등이 이미 쓰던 "코너 고정 순서(`cornerList` API
응답 순서) → `var(--series-N)`" 색상 관례를 이름 기반으로 재사용(VOE
코멘트엔 `corner_id`가 없고 `corner_name`만 있어서). 막대 클릭 시
`selectedCorner` 토글, 선택되지 않은 막대는 `opacity: 0.35`로 흐리게.
우측 리스트는 `selectedCorner`/`selectedMenu`/`selectedClusterTag`
세 조건을 **AND**로 동시 적용(요청 "동시에 적용 가능하게")하고
최신순 정렬, 기본 5건+"전체 N건 보기"로 접는다. 활성 필터는 상단에
`Badge`로 나열하고 "전체 해제" 링크를 둔다.

### 손대지 않는 것 (교차 확인)

- `GET /dashboard/voe-by-category`/`voe-clusters`/`voe-briefing`
  백엔드 엔드포인트·재계산 엔드포인트·응답 스키마 — 전부 무변경. 이번
  라운드는 프론트 전용(백엔드 파일 diff 0줄).
- 고정 카테고리(맛/간/위생/서비스) pill 그리드와 §121의 클러스터
  "네트워크 그래프" 시각화는 이번 5블록 스펙에 자리가 없어 화면에서
  뺐다 — 단, 그 데이터를 만드는 `voeByCategory`/`voeClusters` API와
  재계산 버튼은 그대로 살아있고, 최다 코너/메뉴 계산은 블록1로, 클러스터
  결과 열람은 블록3 태그로, 개별 코멘트 열람은 블록4 리스트로 기능이
  이어진다(정보 손실이 아니라 재배치). `buildVoeClusterGraphOption`
  함수는 더 이상 쓰이지 않아 삭제했다(TS `noUnusedLocals` 걸림).
- `useChartTheme()`의 `surface` 필드(§121에서 추가) — 이번 라운드에선
  안 쓰지만 그대로 둠(다른 훅 소비자에 영향 없음).
- `StatTile`의 기존 `variant="dark"` 호출부 3곳(HomePage 오늘의
  핵심지표, AnalysisPage 예측 요약 2곳) — `size` prop을 안 넘기므로
  폰트 크기 그대로 28px, 완전 무변경.

### 테스트/검증

- 프론트 전용 변경 — `npx tsc -b` + `npx vite build` 클린.
- `pytest` 578개 중 575개 통과, 3개는 오늘(2026-09-01) 날짜에 걸린
  기존 테스트 격리 이슈(`daily_division_stats` unique 제약 충돌,
  `stat_date=2026-09-01`)로 이번 프론트 전용 변경과 무관(백엔드
  파일 무변경 확인) — §120/§121에서도 같은 성격의 날짜 의존 사전
  실패가 있었던 것과 동일 범주.
- **실제 개발 DB(코너 4개·8월 한 달 280건+9월 1일치 seed, 이슈 유형
  3종: 대기시간/짠맛/메뉴 소진) + Playwright**: (1) 8월로 기준월을
  바꿔 총 건수 280, 최다 코너 "한식"(100건), 최다 메뉴 "라면"(46건)
  요약 카드가 다크 강조로 정확히 뜸을 확인, (2) 인사이트 문장이 "최근
  2주 급증(전 2주 대비 +76%) — 한식 코너 관련 의견 집중"으로 자동
  생성되고 라인차트에 급증일 콜아웃(19건×2)이 뜸을 확인, (3) 클러스터
  태그 4개(대기시간/짠맛/메뉴 소진/맛 만족) 노출 확인, (4) "짠맛
  지적" 태그 클릭 → 리스트가 정확히 16건(클러스터 건수와 일치)으로
  필터링됨을 확인 — 최초엔 61건이 잡혀 키워드 오탐 버그를 이 검증
  중 발견·수정(위 "주의" 참고), (5) 코너 막대에서 "분식" 클릭 →
  태그 필터와 AND로 동시 적용돼 필터 배지 2개가 뜨고 리스트가 교집합만
  보임을 확인, (6) "전체 해제" 후 "전체 280건 보기" 클릭 → 필터
  없는 전체 리스트가 펼쳐짐을 확인, (7) 9월(기본 진입 화면)에서 총
  건수 12·"전월 대비 -96%"(녹색 good 톤, 감소=good)가 정확히 뜸을
  확인, (8) 다크 모드 토글 후 전 블록 재확인 — 대비·가독성 문제
  없음. 전 과정 콘솔 에러 0건.
- 문서화(§122) 후 커밋·푸시.

## §123 — 식수추이 기본 기간을 "최근 완결 평일 5일"로 + 코너별 식수 현황 표 복사 버튼

### Context

담당자 요청 2건: (1) 현황 홈 "식수추이"가 기본으로 "이번 주 월~금"을
보여주는데, 오늘이 그 주의 금요일 이전이면 아직 지나지 않은 미래 날짜가
범위에 섞여 빈 데이터로 보인다 — 항상 데이터가 꽉 찬 "가장 최근 완결
평일 5일"을 기본으로 원함(예시: 오늘이 9/1이면 8/24~28). (2) "코너별
조식/중식/석식 식수 현황" 표를 그대로(엑셀에 붙여넣기 가능한 형태로)
복사하는 버튼을 표 오른쪽 위에 추가.

### 설계

**1. 식수추이 기본 기간.** `lib/week.ts`에 `lastCompleteWeekdayMonday()`
추가 — 오늘을 제외하고 어제부터 거슬러 올라가며 가장 최근 금요일을 찾은
뒤, 그 금요일이 속한 주의 월요일을 `mondayOf()`로 반환한다. 오늘이
금요일이어도 그날 데이터가 아직 안 끝났을 수 있다고 보고 항상 지난
완결 주부터 센다(오늘이 화요일 9/1이면 지난 금요일 8/28 → 그 주 월요일
8/24). `HomePage.tsx`의 `trendPeriodStart`/`trendPeriodEnd` 초기값을
기존 `currentMonday()`/`+4일`에서 이 함수 기반으로 교체 — 5일 범위라
기존 daily-fallback 조건(`trendRangeDays < 7`, §104)에 그대로 걸려
daily granularity로 동작하는 건 무변경.

**2. 코너별 식수 현황 표 복사 버튼.** 표가 실제로 렌더링하는 값과 동일한
순서·포맷(헤더 2행 + take_in 각 행 + 소계 + take_out(있으면) + 합계,
`headcount.toLocaleString()`/`(share*100).toFixed(1)+"%"`)으로 탭 구분
텍스트(TSV)를 만드는 순수 함수 `buildCornerMealTypeHeadcountTsv()`를
추가하고, 표 카드 헤더(날짜 피커가 있던 줄)를 `justify-between`으로 바꿔
오른쪽에 "표 복사" 버튼을 놓았다. 클릭 시 `navigator.clipboard.writeText()`
로 복사하고 1.5초간 라벨이 "복사됨"으로 바뀐다 — TSV는 엑셀에 붙여넣으면
탭이 열 구분자로 인식돼 표가 그대로 재현된다. 이 코드베이스에 클립보드
복사 패턴이 없어 새로 만들었다(기존 `Button variant="secondary"` 재사용).

### 손대지 않는 것 (교차 확인)

- 백엔드 `headcount_trend`/`corner_meal_type_headcount` 엔드포인트 —
  무변경. 프론트 기본값·표시만 바뀐다.
- `trendGranularity`/`effectiveTrendGranularity`의 daily-fallback 로직
  (§104), 표의 정렬·소계·합계 계산 로직 — 무변경.
- `currentMonday()`(`lib/week.ts`) 자체는 그대로 두고 새 함수만 추가 —
  다른 화면(예: `AnalysisPage.tsx`의 VOE 탭 §122)이 여전히 쓰고 있어
  삭제하지 않았다. `HomePage.tsx`에서만 이 함수를 더 이상 안 써서
  import만 뺐다.
...
- 프론트 전용 변경(백엔드 파일 무변경) — `npx tsc -b` + `npx vite build`
  클린.
- **실제 개발 DB(컨테이너 날짜 2026-09-01, 화요일) + Playwright**: (1)
  홈 화면 기본 진입 시 "조회 기간"이 정확히 08/24/2026~08/28/2026로
  뜨고 5개 막대(08-24~08-28)에 전부 실제 식수(15/16/18/18/21)가 채워져
  있음을 확인(빈 미래 날짜 없음), (2) "코너별 조식/중식/석식 식수 현황"
  표의 "표 복사" 버튼을 클릭 → 클립보드 내용을
  `navigator.clipboard.readText()`로 읽어 헤더 2행+코너 4행+소계+합계가
...
## §126 — 식수추이 코너 필터: "빈 배열=전체" 상태에서 버튼 표시가 실제와 어긋나던 버그

### Context

담당자 신고: "식수추이에서 테이크아웃은 제외인데 디폴트에도 그래프에
표시되고있음" → 확인 질문에 "총식수는 다 보여주되 막대그래프에서는
테이크아웃을 제외하고싶어 지금 버튼은 꺼져있는데 막대그래프에는
나타남"으로 구체화됐다.

`frontend/src/pages/HomePage.tsx`를 추적해 정확한 원인을 찾았다 —
백엔드 `headcount_trend`의 `corner_ids` 필터 자체는 정상이었다. 진짜
원인은 프론트 상태 설계: `trendCornerIds: number[]`가 빈 배열(`[]`)로
**두 가지 다른 의미**를 표현했다 — (a) 기본값 초기화 전, (b) "전체"
리셋 버튼을 눌러 명시적으로 필터를 해제한 상태. 두 경우 다 쿼리 쪽에선
`corner_ids: trendCornerIds.length > 0 ? trendCornerIds : undefined`로
"필터 없음 = 전체 코너 표시"가 돼 Take Out을 포함한 모든 코너가 실제로
막대에 나온다. 문제는 버튼 쪽: `active = trendCornerIds.includes(...)`는
빈 배열일 때 **모든 버튼을 꺼진 것처럼** 그렸다 — 실제론 전체가 다
보이는데 화면엔 전부 꺼진 것처럼 보이는 정반대 상태. `DEFAULT_TREND_
CORNER_NAMES` 초기화(§81)가 정상 매칭되면 최초 진입 시 이 문제가 안
보이지만, "전체" 리셋 버튼을 한 번이라도 누르면(또는 실제 배포 환경의
어떤 이유로 초기 매칭이 안 되면) 빈 배열 상태로 돌아가 버튼-그래프
불일치가 재현된다.

### 설계

`totalHeadcountTrend`("총식수" 검은 선)는 손대지 않았다 — 담당자가
"총식수는 다 보여주되"라고 명시했고, 이건 2026-08에 이미 "필터와
무관하게 항상 전체"로 만들어진 의도된 동작이다. 백엔드 필터 로직과
프론트 쿼리 호출부의 "빈 배열 = 필터 없음(전체 표시)" 의미도 그대로
뒀다 — **버튼 쪽 표시·토글 로직만** 그 의미에 맞게 고쳤다
(`HomePage.tsx`의 코너 필터 버튼 블록):

1. **활성 표시**: `const active = trendCornerIds.length === 0 ||
   trendCornerIds.includes(c.corner_id)` — 빈 배열(전체 표시 중)일 땐
   모든 버튼을 활성으로 그린다.
2. **토글 클릭**: 빈 배열(암묵적 "전체 선택") 상태에서 코너 하나를
   끄면, 전체 코너 목록에서 그 코너만 뺀 **명시적 목록**으로 전환한다
   (`allIds = cornerListQuery.data.map(...)`, `effective = cur.length
   === 0 ? allIds : cur`, 그 다음 `effective`에서 토글). 이전엔 빈
   배열에서 그냥 `[...cur, id]`로 추가해버려서, Take Out을 끄려는
   클릭이 오히려 "Take Out만 보기"로 뒤집혀 동작했다.
3. **"전체" 리셋 버튼**(`setTrendCornerIds([])`)은 무변경 — 1번
   수정으로 이 상태가 이제 "모든 버튼 활성"으로 정확히 표시되고, 실제로도
   전체가 보이므로 더는 모순이 없다.

### 손대지 않는 것 (교차 확인)

- 백엔드 `headcount_trend`의 `corner_ids` 필터 로직(`analysis.py:283-284`) —
  무변경, 이미 올바르게 동작했다.
- `totalHeadcountTrend`("총식수") — 무변경(담당자 명시 요청).
- `DEFAULT_TREND_CORNER_NAMES`/초기화 `useEffect`(§81) — 무변경.
- 회사구분/끼니 필터 버튼 — 구조는 비슷하지만 이번 신고 대상이 아니라
  손대지 않았다.

### 테스트/검증

- `npx tsc -b` + `npx vite build` 클린.
- **실제 개발 DB(Take Out 코너 신규 시딩, 최근 4일치 취식 데이터) +
  Playwright**: (1) "최근 1주" 기간으로 전환해 Take Out 데이터가 포함된
  구간을 보이게 한 뒤, Take Out 필터 버튼이 활성으로 표시되고
  막대그래프에도 Take Out 구간(파란색, 범례 포함)이 정확히 나타남을
  확인, (2) Take Out 버튼을 클릭해 끔 → Take Out 버튼만 비활성으로
  바뀌고 막대그래프에서 Take Out 구간이 사라지되 한식/분식/양식/중식
  버튼은 계속 활성 상태로 남고 막대에도 그대로 남아있음을 확인(이전
  버그였다면 "Take Out만 보기"로 잘못 전환됐을 상황), (3) "총식수"
  검은 선은 Take Out을 끈 뒤에도 막대 합계보다 높은 값을 유지함을
  확인(요청대로 총식수는 필터와 무관하게 전체 유지). 콘솔 에러 0건.
- 문서화(§126) 후 커밋·푸시.
